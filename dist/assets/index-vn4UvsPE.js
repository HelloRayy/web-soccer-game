...
`)}const E=hw(p.selector);if(E&&(u+=`  - ${E}
...
`)}
    ...
    `,document.head.appendChild(A),()=>{const T=document.getElementById("feedback-cursor-styles");T&&T.remove()}},[E]),b.useEffect(()=>{if(aa!==null&&E)return document.documentElement.setAttribute("data-drawing-hover",""),()=>document.documentElement.removeAttribute("data-drawing-hover")},[aa,E]),b.useEffect(()=>{if(!E||W||es||Ae)return;const S=M=>{const A=M.composedPath()[0]||M.target;if(us(A,"[data-feedback-toolbar]")){ke(null);return}const T=Qi(M.clientX,M.clientY);if(!T||us(T,"[data-feedback-toolbar]")){ke(null);return}const{name:$,elementName:K,path:ge,reactComponents:we}=Pf(T,Vs),je=T.getBoundingClientRect();ke({element:$,elementName:K,elementPath:ge,rect:je,reactComponents:we}),ue({x:M.clientX,y:M.clientY})};return document.addEventListener("mousemove",S),()=>document.removeEventListener("mousemove",S)},[E,W,es,Ae,Vs,rn]);const _a=b.useCallback(S=>{var M;if(Te(S),Qe(null),ft(null),ct([]),(M=S.elementBoundingBoxes)!=null&&M.length){const A=[];for(const T of S.elementBoundingBoxes){const $=T.x+T.width/2,K=T.y+T.height/2-window.scrollY,ge=Qi($,K);ge&&A.push(ge)}Ye(A),Ve(null)}else if(S.boundingBox){const A=S.boundingBox,T=A.x+A.width/2,$=S.isFixed?A.y+A.height/2:A.y+A.height/2-window.scrollY,K=Qi(T,$);if(K){const ge=K.getBoundingClientRect(),we=ge.width/A.width,je=ge.height/A.height;we<.5||je<.5?Ve(null):Ve(K)}else Ve(null);Ye([])}else Ve(null),Ye([])},[]);b.useEffect(()=>{if(!E||es||Ae)return;const S=M=>{var qt,gn;if(ua.current){ua.current=!1;return}const A=M.composedPath()[0]||M.target;if(us(A,"[data-feedback-toolbar]")||us(A,"[data-annotation-popup]")||us(A,"[data-annotation-marker]"))return;if(M.metaKey&&M.shiftKey&&!W&&!V){M.preventDefault(),M.stopPropagation();const At=Qi(M.clientX,M.clientY);if(!At)return;const hn=At.getBoundingClientRect(),{name:ot,path:mt,reactComponents:an}=Pf(At,Vs),Qt=pn.findIndex(Mn=>Mn.element===At);Qt>=0?co(Mn=>Mn.filter((Wn,wn)=>wn!==Qt)):co(Mn=>[...Mn,{element:At,rect:hn,name:ot,path:mt,reactComponents:an??void 0}]);return}const T=us(A,"button, a, input, select, textarea, [role='button'], [onclick]");if(kt.blockInteractions&&T&&(M.preventDefault(),M.stopPropagation()),W){if(T&&!kt.blockInteractions)return;M.preventDefault(),(qt=kr.current)==null||qt.shake();return}if(V){if(T&&!kt.blockInteractions)return;M.preventDefault(),(gn=Cr.current)==null||gn.shake();return}M.preventDefault();const $=Qi(M.clientX,M.clientY);if(!$)return;const{name:K,path:ge,reactComponents:we}=Pf($,Vs),je=$.getBoundingClientRect(),Ie=M.clientX/window.innerWidth*100,We=Uf($),at=We?M.clientY:M.clientY+window.scrollY,bt=window.getSelection();let Ze;bt&&bt.toString().trim().length>0&&(Ze=bt.toString().trim().slice(0,500));const Se=Xc($),tn=Fc($);be({x:Ie,y:at,clientY:M.clientY,element:K,elementPath:ge,selectedText:Ze,boundingBox:{x:je.left,y:We?je.top:je.top+window.scrollY,width:je.width,height:je.height},nearbyText:er($),cssClasses:tr($),isFixed:We,fullPath:Wc($),accessibility:Gc($),computedStyles:tn,computedStylesObj:Se,nearbyElements:Yc($),reactComponents:we??void 0,sourceFile:Jc($),targetElement:$}),ke(null)};return document.addEventListener("click",S,!0),()=>document.removeEventListener("click",S,!0)},[E,es,Ae,W,V,kt.blockInteractions,Vs,pn]),b.useEffect(()=>{if(!E)return;const S=T=>{T.key==="Meta"&&(Gn.current.cmd=!0),T.key==="Shift"&&(Gn.current.shift=!0)},M=T=>{const $=Gn.current.cmd&&Gn.current.shift;T.key==="Meta"&&(Gn.current.cmd=!1),T.key==="Shift"&&(Gn.current.shift=!1);const K=Gn.current.cmd&&Gn.current.shift;$&&!K&&pn.length>0&&Tr()},A=()=>{Gn.current={cmd:!1,shift:!1},co([])};return document.addEventListener("keydown",S),document.addEventListener("keyup",M),window.addEventListener("blur",A),()=>{document.removeEventListener("keydown",S),document.removeEventListener("keyup",M),window.removeEventListener("blur",A)}},[E,pn,Tr]),b.useEffect(()=>{if(!E||W||es||Ae)return;const S=M=>{const A=M.composedPath()[0]||M.target;us(A,"[data-feedback-toolbar]")||us(A,"[data-annotation-marker]")||us(A,"[data-annotation-popup]")||new Set(["P","SPAN","H1","H2","H3","H4","H5","H6","LI","TD","TH","LABEL","BLOCKQUOTE","FIGCAPTION","CAPTION","LEGEND","DT","DD","PRE","CODE","EM","STRONG","B","I","U","S","A","TIME","ADDRESS","CITE","Q","ABBR","DFN","MARK","SMALL","SUB","SUP"]).has(A.tagName)||A.isContentEditable||(M.preventDefault(),_o.current={x:M.clientX,y:M.clientY})};return document.addEventListener("mousedown",S),()=>document.removeEventListener("mousedown",S)},[E,W,es,Ae]),b.useEffect(()=>{if(!E||W)return;const S=M=>{if(!_o.current)return;const A=M.clientX-_o.current.x,T=M.clientY-_o.current.y,$=A*A+T*T,K=ll*ll;if(!Zs&&$>=K&&(ts.current=_o.current,ca(!0),M.preventDefault()),(Zs||$>=K)&&ts.current){if(mo.current){const ot=Math.min(ts.current.x,M.clientX),mt=Math.min(ts.current.y,M.clientY),an=Math.abs(M.clientX-ts.current.x),Qt=Math.abs(M.clientY-ts.current.y);mo.current.style.transform=`translate(${ot}px, ${mt}px)`,mo.current.style.width=`${an}px`,mo.current.style.height=`${Qt}px`}const ge=Date.now();if(ge-Sr.current<Su)return;Sr.current=ge;const we=ts.current.x,je=ts.current.y,Ie=Math.min(we,M.clientX),We=Math.min(je,M.clientY),at=Math.max(we,M.clientX),bt=Math.max(je,M.clientY),Ze=(Ie+at)/2,Se=(We+bt)/2,tn=new Set,qt=[[Ie,We],[at,We],[Ie,bt],[at,bt],[Ze,Se],[Ze,We],[Ze,bt],[Ie,Se],[at,Se]];for(const[ot,mt]of qt){const an=document.elementsFromPoint(ot,mt);for(const Qt of an)Qt instanceof HTMLElement&&tn.add(Qt)}const gn=document.querySelectorAll("button, a, input, img, p, h1, h2, h3, h4, h5, h6, li, label, td, th, div, span, section, article, aside, nav");for(const ot of gn)if(ot instanceof HTMLElement){const mt=ot.getBoundingClientRect(),an=mt.left+mt.width/2,Qt=mt.top+mt.height/2,Mn=an>=Ie&&an<=at&&Qt>=We&&Qt<=bt,Wn=Math.min(mt.right,at)-Math.max(mt.left,Ie),wn=Math.min(mt.bottom,bt)-Math.max(mt.top,We),Cs=Wn>0&&wn>0?Wn*wn:0,_s=mt.width*mt.height,Bs=_s>0?Cs/_s:0;(Mn||Bs>.5)&&tn.add(ot)}const At=[],hn=new Set(["BUTTON","A","INPUT","IMG","P","H1","H2","H3","H4","H5","H6","LI","LABEL","TD","TH","SECTION","ARTICLE","ASIDE","NAV"]);for(const ot of tn){if(us(ot,"[data-feedback-toolbar]")||us(ot,"[data-annotation-marker]"))continue;const mt=ot.getBoundingClientRect();if(!(mt.width>window.innerWidth*.8&&mt.height>window.innerHeight*.5)&&!(mt.width<10||mt.height<10)&&mt.left<at&&mt.right>Ie&&mt.top<bt&&mt.bottom>We){const an=ot.tagName;let Qt=hn.has(an);if(!Qt&&(an==="DIV"||an==="SPAN")){const Mn=ot.textContent&&ot.textContent.trim().length>0,Wn=ot.onclick!==null||ot.getAttribute("role")==="button"||ot.getAttribute("role")==="link"||ot.classList.contains("clickable")||ot.hasAttribute("data-clickable");(Mn||Wn)&&!ot.querySelector("p, h1, h2, h3, h4, h5, h6, button, a")&&(Qt=!0)}if(Qt){let Mn=!1;for(const Wn of At)if(Wn.left<=mt.left&&Wn.right>=mt.right&&Wn.top<=mt.top&&Wn.bottom>=mt.bottom){Mn=!0;break}Mn||At.push(mt)}}}if(Zo.current){const ot=Zo.current;for(;ot.children.length>At.length;)ot.removeChild(ot.lastChild);At.forEach((mt,an)=>{let Qt=ot.children[an];Qt||(Qt=document.createElement("div"),Qt.className=_e.selectedElementHighlight,ot.appendChild(Qt)),Qt.style.transform=`translate(${mt.left}px, ${mt.top}px)`,Qt.style.width=`${mt.width}px`,Qt.style.height=`${mt.height}px`})}}};return document.addEventListener("mousemove",S,{passive:!0}),()=>document.removeEventListener("mousemove",S)},[E,W,Zs,ll]),b.useEffect(()=>{if(!E)return;const S=M=>{const A=Zs,T=ts.current;if(Zs&&T){ua.current=!0;const $=Math.min(T.x,M.clientX),K=Math.min(T.y,M.clientY),ge=Math.max(T.x,M.clientX),we=Math.max(T.y,M.clientY),je=[];document.querySelectorAll("button, a, input, img, p, h1, h2, h3, h4, h5, h6, li, label, td, th").forEach(Ze=>{if(!(Ze instanceof HTMLElement)||us(Ze,"[data-feedback-toolbar]")||us(Ze,"[data-annotation-marker]"))return;const Se=Ze.getBoundingClientRect();Se.width>window.innerWidth*.8&&Se.height>window.innerHeight*.5||Se.width<10||Se.height<10||Se.left<ge&&Se.right>$&&Se.top<we&&Se.bottom>K&&je.push({element:Ze,rect:Se})});const We=je.filter(({element:Ze})=>!je.some(({element:Se})=>Se!==Ze&&Ze.contains(Se))),at=M.clientX/window.innerWidth*100,bt=M.clientY+window.scrollY;if(We.length>0){const Ze=We.reduce((hn,{rect:ot})=>({left:Math.min(hn.left,ot.left),top:Math.min(hn.top,ot.top),right:Math.max(hn.right,ot.right),bottom:Math.max(hn.bottom,ot.bottom)}),{left:1/0,top:1/0,right:-1/0,bottom:-1/0}),Se=We.slice(0,5).map(({element:hn})=>Va(hn).name).join(", "),tn=We.length>5?` +${We.length-5} more`:"",qt=We[0].element,gn=Xc(qt),At=Fc(qt);be({x:at,y:bt,clientY:M.clientY,element:`${We.length} elements: ${Se}${tn}`,elementPath:"multi-select",boundingBox:{x:Ze.left,y:Ze.top+window.scrollY,width:Ze.right-Ze.left,height:Ze.bottom-Ze.top},isMultiSelect:!0,fullPath:Wc(qt),accessibility:Gc(qt),computedStyles:At,computedStylesObj:gn,nearbyElements:Yc(qt),cssClasses:tr(qt),nearbyText:er(qt),sourceFile:Jc(qt)})}else{const Ze=Math.abs(ge-$),Se=Math.abs(we-K);Ze>20&&Se>20&&be({x:at,y:bt,clientY:M.clientY,element:"Area selection",elementPath:`region at (${Math.round($)}, ${Math.round(K)})`,boundingBox:{x:$,y:K+window.scrollY,width:Ze,height:Se},isMultiSelect:!0})}ke(null)}else A&&(ua.current=!0);_o.current=null,ts.current=null,ca(!1),Zo.current&&(Zo.current.innerHTML="")};return document.addEventListener("mouseup",S),()=>document.removeEventListener("mouseup",S)},[E,Zs]);const zs=b.useCallback(async(S,M,A)=>{const T=kt.webhookUrl||C;if(!T||!kt.webhooksEnabled&&!A)return!1;try{return(await fetch(T,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({event:S,timestamp:Date.now(),url:typeof window<"u"?window.location.href:void 0,...M})})).ok}catch($){return console.warn("[Agentation] Webhook failed:",$),!1}},[C,kt.webhookUrl,kt.webhooksEnabled]),Cu=b.useCallback(S=>{var A;if(!W)return;const M={id:Date.now().toString(),x:W.x,y:W.y,comment:S,element:W.element,elementPath:W.elementPath,timestamp:Date.now(),selectedText:W.selectedText,boundingBox:W.boundingBox,nearbyText:W.nearbyText,cssClasses:W.cssClasses,isMultiSelect:W.isMultiSelect,isFixed:W.isFixed,fullPath:W.fullPath,accessibility:W.accessibility,computedStyles:W.computedStyles,nearbyElements:W.nearbyElements,reactComponents:W.reactComponents,sourceFile:W.sourceFile,elementBoundingBoxes:W.elementBoundingBoxes,...p&&on?{sessionId:on,url:typeof window<"u"?window.location.href:void 0,status:"pending"}:{}};J(T=>[...T,M]),da.current=M.id,tt(()=>{da.current=null},300),tt(()=>{ho(T=>new Set(T).add(M.id))},250),l==null||l(M),zs("annotation.add",{annotation:M}),Ls(!0),tt(()=>{be(null),Ls(!1)},150),(A=window.getSelection())==null||A.removeAllRanges(),p&&on&&Fa(p,on,M).then(T=>{T.id!==M.id&&(J($=>$.map(K=>K.id===M.id?{...K,id:T.id}:K)),ho($=>{const K=new Set($);return K.delete(M.id),K.add(T.id),K}))}).catch(T=>{console.warn("[Agentation] Failed to sync annotation:",T)})},[W,l,zs,p,on]),dl=b.useCallback(()=>{Ls(!0),tt(()=>{be(null),Ls(!1)},150)},[]),fl=b.useCallback(S=>{const M=B.findIndex(T=>T.id===S),A=B[M];(V==null?void 0:V.id)===S&&(Os(!0),tt(()=>{Te(null),Ve(null),Ye([]),Os(!1)},150)),dn(S),Ai(T=>new Set(T).add(S)),A&&(c==null||c(A),zs("annotation.delete",{annotation:A})),p&&ki(p,S).catch(T=>{console.warn("[Agentation] Failed to delete annotation from server:",T)}),tt(()=>{J(T=>T.filter($=>$.id!==S)),Ai(T=>{const $=new Set(T);return $.delete(S),$}),dn(null),M<B.length-1&&(mn(M),tt(()=>mn(null),200))},150)},[B,V,c,zs,p]),ma=b.useCallback(S=>{var M;if(!S){Qe(null),ft(null),ct([]);return}if(Qe(S.id),(M=S.elementBoundingBoxes)!=null&&M.length){const A=[];for(const T of S.elementBoundingBoxes){const $=T.x+T.width/2,K=T.y+T.height/2-window.scrollY,we=document.elementsFromPoint($,K).find(je=>!je.closest("[data-annotation-marker]")&&!je.closest("[data-agentation-root]"));we&&A.push(we)}ct(A),ft(null)}else if(S.boundingBox){const A=S.boundingBox,T=A.x+A.width/2,$=S.isFixed?A.y+A.height/2:A.y+A.height/2-window.scrollY,K=Qi(T,$);if(K){const ge=K.getBoundingClientRect(),we=ge.width/A.width,je=ge.height/A.height;we<.5||je<.5?ft(null):ft(K)}else ft(null);ct([])}else ft(null),ct([])},[]),ju=b.useCallback(S=>{if(!V)return;const M={...V,comment:S};J(A=>A.map(T=>T.id===V.id?M:T)),u==null||u(M),zs("annotation.update",{annotation:M}),p&&S0(p,V.id,{comment:S}).catch(A=>{console.warn("[Agentation] Failed to update annotation on server:",A)}),Os(!0),tt(()=>{Te(null),Ve(null),Ye([]),Os(!1)},150)},[V,u,zs,p]),hl=b.useCallback(()=>{Os(!0),tt(()=>{Te(null),Ve(null),Ye([]),Os(!1)},150)},[]),ks=b.useCallback(()=>{const S=B.length,M=De.length>0||!!Ee;if(S===0&&rn.length===0&&!M)return;if(_==null||_(B),zs("annotations.clear",{annotations:B}),p){Promise.all(B.map($=>ki(p,$.id).catch(K=>{console.warn("[Agentation] Failed to delete annotation from server:",K)})));for(const[,$]of Wo.current)$&&ki(p,$).catch(()=>{});Wo.current.clear();for(const[,$]of Ti.current)$&&ki(p,$).catch(()=>{});Ti.current.clear()}dt(!0),qe(!0),gr([]);const A=qo.current;if(A){const $=A.getContext("2d");$&&$.clearRect(0,0,A.width,A.height)}(De.length>0||Ee)&&(ao($=>$+1),nl($=>$+1),tt(()=>{yt([]),Jn(null)},200)),Fe&&Ot(!1),ln&&An(""),io.current={rearrange:null,placements:[]},Vc(et);const T=S*30+200;tt(()=>{J([]),ho(new Set),localStorage.removeItem(mu(et)),dt(!1)},T),tt(()=>qe(!1),1500)},[et,B,rn,De,Ee,Fe,ln,_,zs,p]),_l=b.useCallback(async()=>{const S=typeof window<"u"?window.location.pathname+window.location.search+window.location.hash:et,M=Ae&&Fe;let A;if(M){if(De.length===0&&!Ee&&!ln)return;A=""}else{if(A=j0(B,S,kt.outputDetail),!A&&rn.length===0&&De.length===0&&!Ee)return;A||(A=`## Page Feedback: ${S}
`)}if(!M&&rn.length>0){const T=new Set;for(const we of B)we.drawingIndex!=null&&T.add(we.drawingIndex);const $=qo.current;$&&($.style.visibility="hidden");const K=[],ge=window.scrollY;for(let we=0;we<rn.length;we++){if(T.has(we))continue;const je=rn[we];if(je.points.length<2)continue;const Ie=je.fixed?je.points:je.points.map(yn=>({x:yn.x,y:yn.y-ge}));let We=1/0,at=1/0,bt=-1/0,Ze=-1/0;for(const yn of Ie)We=Math.min(We,yn.x),at=Math.min(at,yn.y),bt=Math.max(bt,yn.x),Ze=Math.max(Ze,yn.y);const Se=bt-We,tn=Ze-at,qt=Math.hypot(Se,tn),gn=Ie[0],At=Ie[Ie.length-1],hn=Math.hypot(At.x-gn.x,At.y-gn.y);let ot;const mt=hn<qt*.35,an=Se/Math.max(tn,1);if(mt&&qt>20){const yn=Math.max(Se,tn)*.15;let ns=0;for(const go of Ie){const Or=go.x-We<yn,zr=bt-go.x<yn,Mu=go.y-at<yn,Eu=Ze-go.y<yn;(Or||zr)&&(Mu||Eu)&&ns++}ot=ns>Ie.length*.15?"box":"circle"}else an>3&&tn<40?ot="underline":hn>qt*.5?ot="arrow":ot="drawing";const Qt=Math.min(10,Ie.length),Mn=Math.max(1,Math.floor(Ie.length/Qt)),Wn=new Set,wn=[],Cs=[gn];for(let yn=Mn;yn<Ie.length-1;yn+=Mn)Cs.push(Ie[yn]);Cs.push(At);for(const yn of Cs){const ns=Qi(yn.x,yn.y);if(!ns||Wn.has(ns)||us(ns,"[data-feedback-toolbar]"))continue;Wn.add(ns);const{name:go}=Va(ns);wn.includes(go)||wn.push(go)}const _s=`${Math.round(We)},${Math.round(at)} → ${Math.round(bt)},${Math.round(Ze)}`;let Bs;(ot==="circle"||ot==="box")&&wn.length>0?Bs=`${ot==="box"?"Boxed":"Circled"} **${wn[0]}**${wn.length>1?` (and ${wn.slice(1).join(", ")})`:""} (region: ${_s})`:ot==="underline"&&wn.length>0?Bs=`Underlined **${wn[0]}** (${_s})`:ot==="arrow"&&wn.length>=2?Bs=`Arrow from **${wn[0]}** to **${wn[wn.length-1]}** (${Math.round(gn.x)},${Math.round(gn.y)} → ${Math.round(At.x)},${Math.round(At.y)})`:wn.length>0?Bs=`${ot==="arrow"?"Arrow":"Drawing"} near **${wn.join("**, **")}** (region: ${_s})`:Bs=`Drawing at ${_s}`,K.push(Bs)}$&&($.style.visibility=""),K.length>0&&(A+=`
**Drawings:**
`,K.forEach((we,je)=>{A+=`${je+1}. ${we}
`}))}if((De.length>0||M&&ln)&&(A+=`
`+x0(De,{width:window.innerWidth,height:window.innerHeight},{blankCanvas:Fe,wireframePurpose:ln||void 0},kt.outputDetail)),Ee){const T=b0(Ee,kt.outputDetail,{width:window.innerWidth,height:window.innerHeight});T&&(A+=`
`+T)}if(m)try{await navigator.clipboard.writeText(A)}catch{}g==null||g(A),D(!0),tt(()=>D(!1),2e3),kt.autoClearAfterCopy&&tt(()=>ks(),500)},[B,rn,De,Ee,Fe,Ae,vs,ln,et,kt.outputDetail,Vs,kt.autoClearAfterCopy,ks,m,g]),pa=b.useCallback(async()=>{const S=typeof window<"u"?window.location.pathname+window.location.search+window.location.hash:et;let M=j0(B,S,kt.outputDetail);if(!M&&De.length===0&&!Ee)return;if(M||(M=`## Page Feedback: ${S}
`),De.length>0&&(M+=`
`+x0(De,{width:window.innerWidth,height:window.innerHeight},{blankCanvas:Fe,wireframePurpose:ln||void 0},kt.outputDetail)),Ee){const T=b0(Ee,kt.outputDetail,{width:window.innerWidth,height:window.innerHeight});T&&(M+=`
`+T)}h&&h(M,B),pe("sending"),await new Promise(T=>tt(T,150));const A=await zs("submit",{output:M,annotations:B},!0);pe(A?"sent":"failed"),tt(()=>pe("idle"),2500),A&&kt.autoClearAfterCopy&&tt(()=>ks(),500)},[h,zs,B,De,Ee,Fe,vs,et,kt.outputDetail,Vs,kt.autoClearAfterCopy,ks]);b.useEffect(()=>{if(!rs)return;const S=10,M=T=>{const $=T.clientX-rs.x,K=T.clientY-rs.y,ge=Math.sqrt($*$+K*K);if(!Ks&&ge>S&&Vo(!0),Ks||ge>S){let we=rs.toolbarX+$,je=rs.toolbarY+K;const Ie=20,We=337,at=44,Ze=We-(E?ls==="connected"?297:257:44),Se=Ie-Ze,tn=window.innerWidth-Ie-We;we=Math.max(Se,Math.min(tn,we)),je=Math.max(Ie,Math.min(window.innerHeight-at-Ie,je)),Ss({x:we,y:je})}},A=()=>{Ks&&(Ko.current=!0),Vo(!1),la(null)};return document.addEventListener("mousemove",M),document.addEventListener("mouseup",A),()=>{document.removeEventListener("mousemove",M),document.removeEventListener("mouseup",A)}},[rs,Ks,E,ls]);const Rr=b.useCallback(S=>{if(S.target.closest("button")||S.target.closest("[data-agentation-settings-panel]"))return;const M=S.currentTarget.parentElement;if(!M)return;const A=M.getBoundingClientRect(),T=(Bt==null?void 0:Bt.x)??A.left,$=(Bt==null?void 0:Bt.y)??A.top;la({x:S.clientX,y:S.clientY,toolbarX:T,toolbarY:$})},[Bt]);if(b.useEffect(()=>{if(!Bt)return;const S=()=>{let $=Bt.x,K=Bt.y;const je=20-(337-(E?ls==="connected"?297:257:44)),Ie=window.innerWidth-20-337;$=Math.max(je,Math.min(Ie,$)),K=Math.max(20,Math.min(window.innerHeight-44-20,K)),($!==Bt.x||K!==Bt.y)&&Ss({x:$,y:K})};return S(),window.addEventListener("resize",S),()=>window.removeEventListener("resize",S)},[Bt,E,ls]),b.useEffect(()=>{const S=M=>{const A=M.target,T=A.tagName==="INPUT"||A.tagName==="TEXTAREA"||A.isContentEditable;if(M.key==="Escape"){if(Ae){xt?_t(null):ha();return}if(es){ws(!1);return}if(pn.length>0){co([]);return}W||E&&(Un(),O(!1))}if((M.metaKey||M.ctrlKey)&&M.shiftKey&&(M.key==="f"||M.key==="F")){M.preventDefault(),Un(),E?Mr():O(!0);return}if(!(T||M.metaKey||M.ctrlKey)&&((M.key==="p"||M.key==="P")&&(M.preventDefault(),Un(),ul()),(M.key==="l"||M.key==="L")&&(M.preventDefault(),Un(),es&&ws(!1),ce&&ze(!1),W&&dl(),Ae?ha():wt(!0)),(M.key==="h"||M.key==="H")&&B.length>0&&(M.preventDefault(),Un(),z($=>!$)),(M.key==="c"||M.key==="C")&&(B.length>0||De.length>0||Ee)&&(M.preventDefault(),Un(),_l()),(M.key==="x"||M.key==="X")&&(B.length>0||De.length>0||Ee)&&(M.preventDefault(),Un(),ks(),De.length>0&&yt([]),Ee&&Jn(null)),M.key==="s"||M.key==="S")){const $=wo(kt.webhookUrl)||wo(C||"");B.length>0&&$&&Z==="idle"&&(M.preventDefault(),Un(),pa())}};return document.addEventListener("keydown",S),()=>document.removeEventListener("keydown",S)},[E,es,Ae,xt,De,Ee,W,B.length,kt.webhookUrl,C,Z,pa,ul,_l,ks,pn]),!$e||de)return null;const Jo=B.length>0,Co=B.filter(S=>!Di.has(S.id)&&S.kind!=="placement"&&S.kind!=="rearrange"),Dr=Co.length>0,jo=B.filter(S=>Di.has(S.id)),Ar=S=>{const K=S.x/100*window.innerWidth,ge=typeof S.y=="string"?parseFloat(S.y):S.y,we={};window.innerHeight-ge-22-10<80&&(we.top="auto",we.bottom="calc(100% + 10px)");const Ie=K-200/2,We=10;if(Ie<We){const at=We-Ie;we.left=`calc(50% + ${at}px)`}else if(Ie+200>window.innerWidth-We){const at=Ie+200-(window.innerWidth-We);we.left=`calc(50% - ${at}px)`}return we};return eg.createPortal(r.jsxs("div",{ref:xe,style:{display:"contents"},"data-agentation-theme":hs?"dark":"light","data-agentation-accent":kt.annotationColorId,"data-agentation-root":"",children:[r.jsx("div",{className:`${_e.toolbar}${k?` ${k}`:""}`,"data-feedback-toolbar":!0,"data-agentation-toolbar":!0,style:Bt?{left:Bt.x,top:Bt.y,right:"auto",bottom:"auto"}:void 0,children:r.jsxs("div",{className:`${_e.toolbarContainer} ${E?_e.expanded:_e.collapsed} ${wr?_e.entrance:""} ${G?_e.hiding:""} ${!kt.webhooksEnabled&&(wo(kt.webhookUrl)||wo(C||""))?_e.serverConnected:""}`,onClick:E?void 0:S=>{if(Ko.current){Ko.current=!1,S.preventDefault();return}O(!0)},onMouseDown:Rr,role:E?void 0:"button",tabIndex:E?-1:0,title:E?void 0:"Start feedback mode",children:[r.jsxs("div",{className:`${_e.toggleContent} ${E?_e.hidden:_e.visible}`,children:[r.jsx(t2,{size:24}),Dr&&r.jsx("span",{className:`${_e.badge} ${E?_e.fadeOut:""} ${wr?_e.entrance:""}`,children:Co.length})]}),r.jsxs("div",{className:`${_e.controlsContent} ${E?_e.visible:_e.hidden} ${Bt&&Bt.y<100?_e.tooltipBelow:""} ${jt||ce?_e.tooltipsHidden:""} ${yr?_e.tooltipsInSession:""}`,onMouseEnter:Qo,onMouseLeave:wu,children:[r.jsxs("div",{className:`${_e.buttonWrapper} ${Bt&&Bt.x<120?_e.buttonWrapperAlignLeft:""}`,children:[r.jsx("button",{className:_e.controlButton,onClick:S=>{S.stopPropagation(),Un(),ul()},"data-active":Y,children:r.jsx(a2,{size:24,isPaused:Y})}),r.jsxs("span",{className:_e.buttonTooltip,children:[Y?"Resume animations":"Pause animations",r.jsx("span",{className:_e.shortcut,children:"P"})]})]}),r.jsxs("div",{className:_e.buttonWrapper,children:[r.jsx("button",{className:`${_e.controlButton} ${hs?"":_e.light}`,onClick:S=>{S.stopPropagation(),Un(),es&&ws(!1),ce&&ze(!1),W&&dl(),Ae?ha():wt(!0)},"data-active":Ae,style:Ae&&Fe?{color:"#f97316",background:"rgba(249, 115, 22, 0.25)"}:void 0,children:r.jsx(m2,{size:21})}),r.jsxs("span",{className:_e.buttonTooltip,children:[Ae?"Exit layout mode":"Layout mode",r.jsx("span",{className:_e.shortcut,children:"L"})]})]}),r.jsxs("div",{className:_e.buttonWrapper,children:[r.jsx("button",{className:_e.controlButton,onClick:S=>{S.stopPropagation(),Un(),z(!F)},disabled:!Jo||Ae,children:r.jsx(i2,{size:24,isOpen:F})}),r.jsxs("span",{className:_e.buttonTooltip,children:[F?"Hide markers":"Show markers",r.jsx("span",{className:_e.shortcut,children:"H"})]})]}),r.jsxs("div",{className:_e.buttonWrapper,children:[r.jsx("button",{className:`${_e.controlButton} ${Ne?_e.statusShowing:""}`,onClick:S=>{S.stopPropagation(),Un(),_l()},disabled:Ae&&Fe?De.length===0&&!((Nr=Ee==null?void 0:Ee.sections)!=null&&Nr.length):!Jo&&rn.length===0&&De.length===0&&!((Lr=Ee==null?void 0:Ee.sections)!=null&&Lr.length),"data-active":Ne,children:r.jsx(s2,{size:24,copied:Ne,tint:Ae&&Fe&&(De.length>0||(ml=Ee==null?void 0:Ee.sections)!=null&&ml.length)?"#f97316":void 0})}),r.jsxs("span",{className:_e.buttonTooltip,children:[Ae&&Fe?"Copy layout":"Copy feedback",r.jsx("span",{className:_e.shortcut,children:"C"})]})]}),r.jsxs("div",{className:`${_e.buttonWrapper} ${_e.sendButtonWrapper} ${E&&!kt.webhooksEnabled&&(wo(kt.webhookUrl)||wo(C||""))?_e.sendButtonVisible:""}`,children:[r.jsxs("button",{className:`${_e.controlButton} ${Z==="sent"||Z==="failed"?_e.statusShowing:""}`,onClick:S=>{S.stopPropagation(),Un(),pa()},disabled:!Jo||!wo(kt.webhookUrl)&&!wo(C||"")||Z==="sending","data-no-hover":Z==="sent"||Z==="failed",tabIndex:wo(kt.webhookUrl)||wo(C||"")?0:-1,children:[r.jsx(o2,{size:24,state:Z}),Jo&&Z==="idle"&&r.jsx("span",{className:_e.buttonBadge,children:B.length})]}),r.jsxs("span",{className:_e.buttonTooltip,children:["Send Annotations",r.jsx("span",{className:_e.shortcut,children:"S"})]})]}),r.jsxs("div",{className:_e.buttonWrapper,children:[r.jsx("button",{className:_e.controlButton,onClick:S=>{S.stopPropagation(),Un(),ks()},disabled:!Jo&&rn.length===0&&De.length===0&&!((pl=Ee==null?void 0:Ee.sections)!=null&&pl.length),"data-danger":!0,children:r.jsx(r2,{size:24})}),r.jsxs("span",{className:_e.buttonTooltip,children:["Clear all",r.jsx("span",{className:_e.shortcut,children:"X"})]})]}),r.jsxs("div",{className:_e.buttonWrapper,children:[r.jsx("button",{className:_e.controlButton,onClick:S=>{S.stopPropagation(),Un(),Ae&&ha(),ze(!ce)},children:r.jsx(l2,{size:24})}),p&&ls!=="disconnected"&&r.jsx("span",{className:`${_e.mcpIndicator} ${_e[ls]} ${ce?_e.hidden:""}`,title:ls==="connected"?"MCP Connected":"MCP Connecting..."}),r.jsx("span",{className:_e.buttonTooltip,children:"Settings"})]}),r.jsx("div",{className:_e.divider}),r.jsxs("div",{className:`${_e.buttonWrapper} ${Bt&&typeof window<"u"&&Bt.x>window.innerWidth-120?_e.buttonWrapperAlignRight:""}`,children:[r.jsx("button",{className:_e.controlButton,onClick:S=>{S.stopPropagation(),Un(),Mr()},children:r.jsx(c2,{size:24})}),r.jsxs("span",{className:_e.buttonTooltip,children:["Exit",r.jsx("span",{className:_e.shortcut,children:"Esc"})]})]})]}),r.jsx(Xv,{visible:Ae&&E,activeType:xt,onSelect:S=>{_t(xt===S?null:S)},isDarkMode:hs,sectionCount:(Ee==null?void 0:Ee.sections.length)??0,onDetectSections:()=>{const S=tw(),M=(Ee==null?void 0:Ee.sections)??[],A=new Set(M.map(ge=>ge.selector)),T=S.filter(ge=>!A.has(ge.selector)),$=[...M,...T],K=[...(Ee==null?void 0:Ee.originalOrder)??[],...T.map(ge=>ge.id)];Jn({sections:$,originalOrder:K,detectedAt:Date.now()})},placementCount:De.length,onClearPlacements:()=>{ao(S=>S+1),nl(S=>S+1),tt(()=>{Jn({sections:[],originalOrder:[],detectedAt:Date.now()})},200)},blankCanvas:Fe,onBlankCanvasChange:S=>{const M={sections:[],originalOrder:[],detectedAt:Date.now()};S?(tl.current={rearrange:Ee,placements:De},Jn(io.current.rearrange||M),yt(io.current.placements),_t(null)):(io.current={rearrange:Ee,placements:De},Jn(tl.current.rearrange||M),yt(tl.current.placements)),Ot(S)},wireframePurpose:ln,onWireframePurposeChange:An,Tooltip:Ki,onDragStart:(S,M)=>{M.preventDefault();const A=Le[S];let T=null,$=!1;const K=M.clientX,ge=M.clientY,we=M.target.closest("[data-feedback-toolbar]"),je=(we==null?void 0:we.getBoundingClientRect().top)??window.innerHeight,Ie=at=>{const bt=at.clientX-K,Ze=at.clientY-ge;if(!$&&(Math.abs(bt)>4||Math.abs(Ze)>4)&&($=!0,T=document.createElement("div"),T.className=`${ne.dragPreview}${Fe?` ${ne.dragPreviewWireframe}`:""}`,document.body.appendChild(T)),!T)return;const Se=Math.max(0,je-at.clientY),tn=Math.min(1,Se/180),qt=1-Math.pow(1-tn,2),gn=28,At=20,hn=Math.min(140,A.width*.18),ot=Math.min(90,A.height*.18),mt=gn+(hn-gn)*qt,an=At+(ot-At)*qt;T.style.width=`${mt}px`,T.style.height=`${an}px`,T.style.left=`${at.clientX-mt/2}px`,T.style.top=`${at.clientY-an/2}px`,T.style.opacity=`${.5+.5*qt}`,T.textContent=qt>.25?S:""},We=at=>{if(window.removeEventListener("mousemove",Ie),window.removeEventListener("mouseup",We),T&&document.body.removeChild(T),$){const bt=A.width,Ze=A.height,Se=window.scrollY,tn=Math.max(0,at.clientX-bt/2),qt=Math.max(0,at.clientY+Se-Ze/2),gn={id:`dp-${Date.now()}-${Math.random().toString(36).slice(2,7)}`,type:S,x:tn,y:qt,width:bt,height:Ze,scrollY:Se,timestamp:Date.now()};yt(At=>[...At,gn]),_t(null),Go.current=new Set,oa(At=>At+1)}};window.addEventListener("mousemove",Ie),window.addEventListener("mouseup",We)}}),r.jsx(u4,{settings:kt,onSettingsChange:S=>br(M=>({...M,...S})),isDarkMode:hs,onToggleTheme:Qs,isDevMode:ol,connectionStatus:ls,endpoint:p,isVisible:Ce,toolbarNearBottom:!!Bt&&Bt.y<230,settingsPage:it,onSettingsPageChange:ht,onHideToolbar:ku})]})}),(Ae||Mt)&&r.jsx("div",{className:`${ne.blankCanvas} ${vn?ne.visible:""} ${Yt?ne.gridActive:""}`,style:{"--canvas-opacity":In},"data-feedback-toolbar":!0}),Ae&&Fe&&vn&&r.jsxs("div",{className:ne.wireframeNotice,"data-feedback-toolbar":!0,children:[r.jsxs("div",{className:ne.wireframeOpacityRow,children:[r.jsx("span",{className:ne.wireframeOpacityLabel,children:"Toggle Opacity"}),r.jsx("input",{type:"range",className:ne.wireframeOpacitySlider,min:0,max:1,step:.01,value:In,onChange:S=>oo(Number(S.target.value))})]}),r.jsxs("div",{className:ne.wireframeNoticeTitleRow,children:[r.jsx("span",{className:ne.wireframeNoticeTitle,children:"Wireframe Mode"}),r.jsx("span",{className:ne.wireframeNoticeDivider}),r.jsx("button",{className:ne.wireframeStartOver,onClick:()=>{ao(S=>S+1),Jn({sections:[],originalOrder:[],detectedAt:Date.now()}),io.current={rearrange:null,placements:[]},An(""),Vc(et)},children:"Start Over"})]}),"Drag components onto the canvas.",r.jsx("br",{}),"Copied output will only include the wireframed layout."]}),(Ae||Mt)&&r.jsx(Pv,{placements:De,onChange:yt,activeComponent:Mt?null:xt,onActiveComponentChange:_t,isDarkMode:hs,exiting:Mt,onInteractionChange:jn,passthrough:!xt,extraSnapRects:Ee==null?void 0:Ee.sections.map(S=>S.currentRect),deselectSignal:Mi,clearSignal:Ei,wireframe:Fe,onSelectionChange:(S,M)=>{Go.current=S,M||(Ns.current=new Set,Fo(A=>A+1))},onDragMove:(S,M)=>{const A=Ns.current;if(!(!A.size||!Ee)){if(!Xn.current){Xn.current=new Map;for(const T of Ee.sections)A.has(T.id)&&Xn.current.set(T.id,{x:T.currentRect.x,y:T.currentRect.y})}for(const T of Ee.sections){if(!A.has(T.id)||!Xn.current.get(T.id))continue;const K=document.querySelector(`[data-rearrange-section="${T.id}"]`);K&&(K.style.transform=`translate(${S}px, ${M}px)`)}}},onDragEnd:(S,M,A)=>{const T=Ns.current,$=Xn.current;if(Xn.current=null,!(!T.size||!Ee||!$)){for(const K of T){const ge=document.querySelector(`[data-rearrange-section="${K}"]`);ge&&(ge.style.transform="")}A&&Jn(K=>K&&{...K,sections:K.sections.map(ge=>{const we=$.get(ge.id);return we?{...ge,currentRect:{...ge.currentRect,x:Math.max(0,we.x+S),y:Math.max(0,we.y+M)}}:ge})})}}}),(Ae||Mt)&&Ee&&r.jsx(ow,{rearrangeState:Ee,onChange:Jn,isDarkMode:hs,exiting:Mt,blankCanvas:Fe,extraSnapRects:De.map(S=>({x:S.x,y:S.y,width:S.width,height:S.height})),clearSignal:Nn,deselectSignal:Xo,onSelectionChange:(S,M)=>{Ns.current=S,M||(Go.current=new Set,oa(A=>A+1))},onDragMove:(S,M)=>{const A=Go.current;if(A.size){if(!Xn.current){Xn.current=new Map;for(const T of De)A.has(T.id)&&Xn.current.set(T.id,{x:T.x,y:T.y})}for(const T of A){const $=document.querySelector(`[data-design-placement="${T}"]`);$&&($.style.transform=`translate(${S}px, ${M}px)`)}}},onDragEnd:(S,M,A)=>{const T=Go.current,$=Xn.current;if(Xn.current=null,!(!T.size||!$)){for(const K of T){const ge=document.querySelector(`[data-design-placement="${K}"]`);ge&&(ge.style.transform="")}A&&yt(K=>K.map(ge=>{const we=$.get(ge.id);return we?{...ge,x:Math.max(0,we.x+S),y:Math.max(0,we.y+M)}:ge}))}}}),r.jsx("canvas",{ref:qo,className:`${_e.drawCanvas} ${es?_e.active:""}`,style:{opacity:rl?1:0,transition:"opacity 0.15s ease"},"data-feedback-toolbar":!0}),r.jsxs("div",{className:_e.markersLayer,"data-feedback-toolbar":!0,children:[he&&Co.filter(S=>!S.isFixed).map((S,M,A)=>r.jsx(M0,{annotation:S,globalIndex:Co.findIndex(T=>T.id===S.id),layerIndex:M,layerSize:A.length,isExiting:Q,isClearing:rt,isAnimated:al.has(S.id),isHovered:!Q&&en===S.id,isDeleting:Dt===S.id,isEditingAny:!!V,renumberFrom:bn,markerClickBehavior:kt.markerClickBehavior,tooltipStyle:Ar(S),onHoverEnter:T=>!Q&&T.id!==da.current&&ma(T),onHoverLeave:()=>ma(null),onClick:T=>kt.markerClickBehavior==="delete"?fl(T.id):_a(T),onContextMenu:_a},S.id)),he&&!Q&&jo.filter(S=>!S.isFixed).map(S=>r.jsx(E0,{annotation:S},S.id))]}),r.jsxs("div",{className:_e.fixedMarkersLayer,"data-feedback-toolbar":!0,children:[he&&Co.filter(S=>S.isFixed).map((S,M,A)=>r.jsx(M0,{annotation:S,globalIndex:Co.findIndex(T=>T.id===S.id),layerIndex:M,layerSize:A.length,isExiting:Q,isClearing:rt,isAnimated:al.has(S.id),isHovered:!Q&&en===S.id,isDeleting:Dt===S.id,isEditingAny:!!V,renumberFrom:bn,markerClickBehavior:kt.markerClickBehavior,tooltipStyle:Ar(S),onHoverEnter:T=>!Q&&T.id!==da.current&&ma(T),onHoverLeave:()=>ma(null),onClick:T=>kt.markerClickBehavior==="delete"?fl(T.id):_a(T),onContextMenu:_a},S.id)),he&&!Q&&jo.filter(S=>S.isFixed).map(S=>r.jsx(E0,{annotation:S,fixed:!0},S.id))]}),E&&r.jsxs("div",{className:_e.overlay,"data-feedback-toolbar":!0,style:W||V?{zIndex:99999}:void 0,children:[(oe==null?void 0:oe.rect)&&!W&&!St&&!Zs&&r.jsx("div",{className:`${_e.hoverHighlight} ${_e.enter}`,style:{left:oe.rect.left,top:oe.rect.top,width:oe.rect.width,height:oe.rect.height,borderColor:"color-mix(in srgb, var(--agentation-color-accent) 50%, transparent)",backgroundColor:"color-mix(in srgb, var(--agentation-color-accent) 4%, transparent)"}}),pn.filter(S=>document.contains(S.element)).map((S,M)=>{const A=S.element.getBoundingClientRect(),T=pn.length>1;return r.jsx("div",{className:T?_e.multiSelectOutline:_e.singleSelectOutline,style:{position:"fixed",left:A.left,top:A.top,width:A.width,height:A.height,...T?{}:{borderColor:"color-mix(in srgb, var(--agentation-color-accent) 60%, transparent)",backgroundColor:"color-mix(in srgb, var(--agentation-color-accent) 5%, transparent)"}}},M)}),en&&!W&&(()=>{var $;const S=B.find(K=>K.id===en);if(!(S!=null&&S.boundingBox))return null;if(($=S.elementBoundingBoxes)!=null&&$.length)return Ke.length>0?Ke.filter(K=>document.contains(K)).map((K,ge)=>{const we=K.getBoundingClientRect();return r.jsx("div",{className:`${_e.multiSelectOutline} ${_e.enter}`,style:{left:we.left,top:we.top,width:we.width,height:we.height}},`hover-outline-live-${ge}`)}):S.elementBoundingBoxes.map((K,ge)=>r.jsx("div",{className:`${_e.multiSelectOutline} ${_e.enter}`,style:{left:K.x,top:K.y-Re,width:K.width,height:K.height}},`hover-outline-${ge}`));const M=ve&&document.contains(ve)?ve.getBoundingClientRect():null,A=M?{x:M.left,y:M.top,width:M.width,height:M.height}:{x:S.boundingBox.x,y:S.isFixed?S.boundingBox.y:S.boundingBox.y-Re,width:S.boundingBox.width,height:S.boundingBox.height},T=S.isMultiSelect;return r.jsx("div",{className:`${T?_e.multiSelectOutline:_e.singleSelectOutline} ${_e.enter}`,style:{left:A.x,top:A.y,width:A.width,height:A.height,...T?{}:{borderColor:"color-mix(in srgb, var(--agentation-color-accent) 60%, transparent)",backgroundColor:"color-mix(in srgb, var(--agentation-color-accent) 5%, transparent)"}}})})(),oe&&!W&&!St&&!Zs&&r.jsxs("div",{className:`${_e.hoverTooltip} ${_e.enter}`,style:{left:Math.max(8,Math.min(N.x,window.innerWidth-100)),top:Math.max(N.y-(oe.reactComponents?48:32),8)},children:[oe.reactComponents&&r.jsx("div",{className:_e.hoverReactPath,children:oe.reactComponents}),r.jsx("div",{className:_e.hoverElementName,children:oe.elementName})]}),W&&r.jsxs(r.Fragment,{children:[(ei=W.multiSelectElements)!=null&&ei.length?W.multiSelectElements.filter(S=>document.contains(S)).map((S,M)=>{const A=S.getBoundingClientRect();return r.jsx("div",{className:`${_e.multiSelectOutline} ${Ln?_e.exit:_e.enter}`,style:{left:A.left,top:A.top,width:A.width,height:A.height}},`pending-multi-${M}`)}):W.targetElement&&document.contains(W.targetElement)?(()=>{const S=W.targetElement.getBoundingClientRect();return r.jsx("div",{className:`${_e.singleSelectOutline} ${Ln?_e.exit:_e.enter}`,style:{left:S.left,top:S.top,width:S.width,height:S.height,borderColor:"color-mix(in srgb, var(--agentation-color-accent) 60%, transparent)",backgroundColor:"color-mix(in srgb, var(--agentation-color-accent) 5%, transparent)"}})})():W.boundingBox&&r.jsx("div",{className:`${W.isMultiSelect?_e.multiSelectOutline:_e.singleSelectOutline} ${Ln?_e.exit:_e.enter}`,style:{left:W.boundingBox.x,top:W.boundingBox.y-Re,width:W.boundingBox.width,height:W.boundingBox.height,...W.isMultiSelect?{}:{borderColor:"color-mix(in srgb, var(--agentation-color-accent) 60%, transparent)",backgroundColor:"color-mix(in srgb, var(--agentation-color-accent) 5%, transparent)"}}}),(()=>{const S=W.x,M=W.isFixed?W.y:W.y-Re;return r.jsxs(r.Fragment,{children:[r.jsx(e4,{x:S,y:M,isMultiSelect:W.isMultiSelect,isExiting:Ln}),r.jsx(hu,{ref:kr,element:W.element,selectedText:W.selectedText,computedStyles:W.computedStylesObj,placeholder:W.element==="Area selection"?"What should change in this area?":W.isMultiSelect?"Feedback for this group of elements...":"What should change?",onSubmit:Cu,onCancel:dl,isExiting:Ln,lightMode:!hs,accentColor:W.isMultiSelect?"var(--agentation-color-green)":"var(--agentation-color-accent)",style:{left:Math.max(160,Math.min(window.innerWidth-160,S/100*window.innerWidth)),...M>window.innerHeight-290?{bottom:window.innerHeight-M+20}:{top:M+20}}})]})})()]}),V&&r.jsxs(r.Fragment,{children:[(ti=V.elementBoundingBoxes)!=null&&ti.length?nt.length>0?nt.filter(S=>document.contains(S)).map((S,M)=>{const A=S.getBoundingClientRect();return r.jsx("div",{className:`${_e.multiSelectOutline} ${_e.enter}`,style:{left:A.left,top:A.top,width:A.width,height:A.height}},`edit-multi-live-${M}`)}):V.elementBoundingBoxes.map((S,M)=>r.jsx("div",{className:`${_e.multiSelectOutline} ${_e.enter}`,style:{left:S.x,top:S.y-Re,width:S.width,height:S.height}},`edit-multi-${M}`)):(()=>{const S=Ge&&document.contains(Ge)?Ge.getBoundingClientRect():null,M=S?{x:S.left,y:S.top,width:S.width,height:S.height}:V.boundingBox?{x:V.boundingBox.x,y:V.isFixed?V.boundingBox.y:V.boundingBox.y-Re,width:V.boundingBox.width,height:V.boundingBox.height}:null;return M?r.jsx("div",{className:`${V.isMultiSelect?_e.multiSelectOutline:_e.singleSelectOutline} ${_e.enter}`,style:{left:M.x,top:M.y,width:M.width,height:M.height,...V.isMultiSelect?{}:{borderColor:"color-mix(in srgb, var(--agentation-color-accent) 60%, transparent)",backgroundColor:"color-mix(in srgb, var(--agentation-color-accent) 5%, transparent)"}}}):null})(),r.jsx(hu,{ref:Cr,element:V.element,selectedText:V.selectedText,computedStyles:Kv(V.computedStyles),placeholder:"Edit your feedback...",initialValue:V.comment,submitLabel:"Save",onSubmit:ju,onCancel:hl,onDelete:()=>fl(V.id),isExiting:ra,lightMode:!hs,accentColor:V.isMultiSelect?"var(--agentation-color-green)":"var(--agentation-color-accent)",style:(()=>{const S=V.isFixed?V.y:V.y-Re;return{left:Math.max(160,Math.min(window.innerWidth-160,V.x/100*window.innerWidth)),...S>window.innerHeight-290?{bottom:window.innerHeight-S+20}:{top:S+20}}})()})]}),Zs&&r.jsxs(r.Fragment,{children:[r.jsx("div",{ref:mo,className:_e.dragSelection}),r.jsx("div",{ref:Zo,className:_e.highlightsContainer})]})]})]}),document.body)}const h4=()=>{const o=ko();return o.pathname==="/"||o.pathname==="/controller"?null:r.jsx("nav",{className:"bg-slate-900 border-b border-slate-800 text-slate-100 p-4 flex items-center justify-between shadow-lg",children:r.jsxs("div",{className:"flex items-center gap-6",children:[r.jsx(lr,{to:"/",className:"font-extrabold text-xl tracking-tight text-cyan-400 hover:text-cyan-300",children:"⚽ Soccer Web Game"}),r.jsx(lr,{to:"/",className:"text-sm font-semibold hover:text-cyan-400 transition",children:"Arena Game"}),r.jsx(lr,{to:"/controller",className:"text-sm font-semibold hover:text-cyan-400 transition",children:"📱 Mobile Remote"})]})})};function _4(){return r.jsx(vb,{children:r.jsxs("div",{className:"min-h-screen bg-slate-950 text-slate-100 flex flex-col select-none",children:[r.jsx(h4,{}),r.jsxs(Kx,{children:[r.jsx(Yf,{path:"/",element:r.jsx(q5,{})}),r.jsx(Yf,{path:"/controller",element:r.jsx(Q5,{})})]}),r.jsx(f4,{})]})})}K1.createRoot(document.getElementById("root")).render(r.jsx(Rn.StrictMode,{children:r.jsx(_4,{})}));
