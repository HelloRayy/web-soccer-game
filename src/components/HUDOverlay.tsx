...
export const HUDOverlay: React.FC<HUDOverlayProps> = ({
  ...
  return (
    <>
      ...
      {/* INSTANT REPLAY TV BROADCAST WATERMARK & SKIP BUTTON */}
      {isReplayActive && (
        <div className="fixed top-[90px] left-4 z-50 pointer-events-auto flex items-center gap-3 animate-in fade-in duration-200">
          <div className="bg-[#080d14]/95 border-2 border-amber-500/90 px-4 py-2 rounded-2xl backdrop-blur-xl flex items-center gap-2.5 text-xs font-mono text-amber-300 shadow-2xl shadow-amber-500/20">
            <span className="w-3 h-3 rounded-full bg-amber-400 animate-ping" />
            <span className="font-black tracking-widest text-amber-300">🔴 INSTANT REPLAY</span>
            <span className="text-amber-500/60">|</span>
            <span className="text-amber-200 font-bold">SLOW-MOTION 0.6x</span>
          </div>
          {onSkipReplay && (
            <button
              onClick={onSkipReplay}
              className="bg-gradient-to-r from-amber-500 to-yellow-400 hover:brightness-110 text-slate-950 font-black font-mono text-xs px-4 py-2 rounded-2xl shadow-2xl transition cursor-pointer flex items-center gap-1.5 border border-amber-300 active:scale-95"
            >
              <span>⏩</span>
              <span>LEWATI REPLAY (SKIP)</span>
            </button>
          )}
        </div>
      )}
      ...
      {!isReplayActive && arcadeCallouts && arcadeCallouts.length > 0 && (
        <div className="fixed top-[90px] left-1/2 -translate-x-1/2 z-40 pointer-events-none flex flex-col items-center">
      ...
      {showHUD && activePlayer && !isReplayActive && (
      ...
      {showHUD && !isReplayActive && (
        ...
        {goalBannerText && (
          <div className="fixed inset-x-0 top-[90px] z-50 pointer-events-none flex items-center justify-center p-2">
            <div className="bg-[#0a0d08]/97 border-2 border-yellow-400/90 px-5 py-2.5 rounded-2xl shadow-2xl backdrop-blur-xl flex items-center gap-4 animate-in zoom-in-90 duration-150">
              <span className="text-3xl animate-bounce">⚽</span>
              <div className="flex flex-col leading-tight">
                <span className="text-xl sm:text-2xl font-black text-yellow-300 font-mono tracking-widest uppercase drop-shadow-lg">
                  GOAL!
                </span>
                <span className="text-xs sm:text-sm font-bold text-emerald-300 font-mono tracking-wider opacity-90">
                  🚀 {goalBannerText.replace('GOAL! 🚀 ', '')}
                </span>
              </div>
            </div>
          </div>
      ...
      {showHUD && radarData && !isReplayActive && (
