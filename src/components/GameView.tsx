...
export const GameView: React.FC<GameViewProps> = ({
  ...
  useGameLoop((dt) => {
    ...
    // 9. DYNAMIC MANUAL SHOOTING AIM TRAJECTORY LINE & CROSSHAIR INDICATOR
    players.forEach((p) => {
      if (p.hasPossession && !p.id.includes('bot')) {
        const aimAngle = p.currentAimAngle !== undefined ? p.currentAimAngle : p.facingAngle;
        
        // Target Goal parameters
        const targetGoal = p.team === 'home' ? field.goals.awayGoal : field.goals.homeGoal;
        const goalCenterX = targetGoal.x;
        const goalTopY = targetGoal.top;
        const goalBottomY = targetGoal.bottom;

        // Line Trajectory Distance (380px extension)
        const lineLength = 380;
        const endX = ball.pos.x + Math.cos(aimAngle) * lineLength;
        const endY = ball.pos.y + Math.sin(aimAngle) * lineLength;

        // Check if shot trajectory intersects the goal net plane
        let isAimingAtGoal = false;
        if (p.team === 'home' && endX >= goalCenterX - 25) {
          const slope = (endY - ball.pos.y) / ((endX - ball.pos.x) || 1);
          const intersectY = ball.pos.y + slope * (goalCenterX - ball.pos.x);
          if (intersectY >= goalTopY - 15 && intersectY <= goalBottomY + 15) {
            isAimingAtGoal = true;
          }
        } else if (p.team === 'away' && endX <= goalCenterX + 25) {
          const slope = (endY - ball.pos.y) / ((endX - ball.pos.x) || 1);
          const intersectY = ball.pos.y + slope * (goalCenterX - ball.pos.x);
          if (intersectY >= goalTopY - 15 && intersectY <= goalBottomY + 15) {
            isAimingAtGoal = true;
        ...
        const lineColor = isAimingAtGoal ? '#22c55e' : '#ef4444'; // Neon Emerald Green on Target, Red when Wide!

        ctx.save();
        
        // Glowing Dotted Trajectory Line
        ...
        // Target Status Text Badge ('⚽ TARGET' vs '❌ WIDE')
        ctx.font = '900 11px system-ui, sans-serif';
        ctx.textAlign = 'center';
        ctx.fillStyle = lineColor;
        ctx.shadowBlur = 8;
        ctx.fillText(isAimingAtGoal ? '⚽ TARGET' : '❌ WIDE', endX, endY - 20);
